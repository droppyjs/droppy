...
function entryRename(view, entry, wasEmpty, callback) {
  ...
  // Add inline element
  const renamer = $(
    `<input type="text" class="inline-namer" value="${linkText}" placeholder="${linkText}">`,
  ).insertAfter(link)
  renamer
    .off("input")
    .on("input", function () {
  ...
  function configCM(text, filename) {
    ...
    loadContent(view, "document", null, html).then(() => {
      ...
      editor.setOption("extraKeys", {
        Tab(cm) {
          cm.replaceSelection(
            droppy.get("indentWithTabs") ? "\t" : new Array(droppy.get("indentUnit") + 1).join(" "),
          )
        },
        "Cmd-S": saveCM,
        "Ctrl-S": saveCM,
...
function initAudio(view) {
  ...
  bar.off("click").on("click", function (event) {
    const time =
      player.duration * ((event.pageX - bar[0].getBoundingClientRect().left) / bar[0].clientWidth)
