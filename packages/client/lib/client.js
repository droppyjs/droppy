...
function newView(dest, vId) {
    ...
    if (droppy.views.length > 1) {
        droppy.views.forEach((view) => {
            ...
            $(view)
                .find(".newview")[0]
                .setAttribute("aria-label", "Close this view");
        });
    }
...
function destroyView(vId) {
    ...
    droppy.views.forEach((view) => {
        ...
        $(view)
            .find(".newview")[0]
            .setAttribute("aria-label", "Create new view");
        checkPathOverflow($(view));
        view.vId = 0;
        if (view.ps) view.ps.updateSize(true);
    });
        ...
        switch (msg.type) {
            ...
            case "SHARELINK": {
                ...
                if (view.find(".info-box.link.in").length) {
                    view.find(".link-out")[0].textContent = getFullLink(
                        msg.link,
                    );
                } else {
            ...
            case "SAVE_STATUS": {
                ...
                file.removeClass("dirty").addClass(
                    msg.status === 0 ? "saved" : "save-failed",
                );
                setTimeout(() => {
...
function upload(view, fd, files) {
    ...
    if (!files || !files.length) {
        return showError(view, "Unable to upload.");
    }

    view[0].uploadId += 1;

    const id = view[0].uploadId;
    ...
    xhr.upload.addEventListener(
        ...
        throttle((e) => {
            if (e?.lengthComputable)
                uploadProgress(view, id, e.loaded, e.total);
        }, 100),
    );
    ...
    renamer
        ...
        .on("input", function () {
            ...
            const exists = activeFiles.some((file) => {
                return (
                    file ===
                    (droppy.caseSensitive ? input : input.toLowerCase())
                );
            });
...
function stopEdit(view, entry, wasEmpty) {
    ...
    view.find(
        ".inline-namer, .data-row.new-file, .data-row.new-folder",
    ).remove();
    if (wasEmpty)
        view.find(".content").html(
            Handlebars.templates.directory({ entries: [] }),
        );
}
...
function openDirectory(view, data, isSearch) {
    ...
    loadContent(view, "directory", null, html).then(() => {
        ...
        view.find(".data-row .entry-menu")
            .off("click")
            .on("click", (e) => {
                showEntryMenu(
                    $(e.target).parents(".data-row"),
                    e.clientX,
                    e.clientY,
                );
            });
        ...
        view.find(".data-row .zip, .data-row .download, .entry-link.file")
            .off("click")
            .on("click", (e) => {
                e.stopPropagation();
                if (droppy.socketWait) return;
        ...
        view.find(".icon-play, .icon-view")
            .off("click")
            .on("click", function () {
                $(this).parents(".data-row").find(".file-link")[0].click();
            });

        view.find(".header-name, .header-mtime, .header-size")
            .off("click")
            .on("click", function () {
        ...
        view.find(".header-multi-select")
            ...
            .on(
                "click",
                debounce(() => {
                    const selects = view.find("input[name='selected-item']");
        ...
        view.find("input[name='selected-item']")
            .off("change")
            .on("change", (e) => {
                ...
                if (selectedItems.length === 0) {
                    ...
                    document.querySelector(
                        ".header-multi-select svg",
                    ).style.display = "block";

                    document.querySelector(
                        ".header-multi-select input",
                    ).checked = false;
                    return;
                }
...
function loadContent(view, type, mediaType, content) {
    return new Promise((resolve) => {
        ...
        if (view[0].animDirection === "center") {
            ...
            view.find(".content:not(.new)").replaceClass(
                navRegex,
                view[0].animDirection === "forward"
                    ? "back"
                    : view[0].animDirection === "back"
        ...
        if (view[0].dataset.type === "directory") {
            ...
            if (dragData.type === "folder") {
                ...
                if (
                    ...
                    openFile(
                        view,
                        dirname(dragData.path),
                        basename(dragData.path),
                    );
                }
            }
        }
...
function initButtons(view) {
    ...
    view.off("click", ".ad").on("click", ".ad", function (e) {
        ...
        if ($(this).hasClass("disabled")) {
            showError(
                getView(0),
                "Your browser doesn't support directory uploading",
            );
        } else {
    ...
    view.off("click", ".search.toggled-off").on(
        "click",
        ".search.toggled-off",
        function () {
            const search = $(this);
    ...
    view.off("click", ".search.toggled-on svg").on(
        "click",
        ".search.toggled-on svg",
        function () {
            const view = $(this).parents(".view");
            openDirectory(view, view[0].currentData);
        },
    );
    view.off("keyup", ".search input").on(
        "keyup",
        ".search input",
        function (e) {
    ...
    view.off("input", ".search input").on(
        "input",
        ".search input",
        debounce(doSearch, 1000),
    );
    view.off("click", ".globalsearch input").on(
        "click",
        ".globalsearch input",
        (e) => {
            e.stopPropagation();
        ...
        .on("click", (event) => {
            ...
            showConfirmation(
                ...
                () => {
                    ...
                    showSpinner(view);
                    sendMessage(view[0].vId, "DELETE_FILE", [
                        entry[0].dataset.id,
                    ]);
                },
            );
        ...
        loadContent(view, "media", type, html).then(() => {
            ...
            view[0].ps.listen("afterChange", function () {
                ...
                if (type === "pdf") {
                    ...
                    this.currItem.container.style.transformOrigin =
                        "center top"; // center zoom out
            ...
            view[0].ps.listen("preventDragEvent", (_, isDown) => {
                view.find(".pswp__container")[0].classList[
                    isDown ? "add" : "remove"
                ]("no-transition");
            });
        ...
        loadContent(view, "document", null, html).then(() => {
            ...
            view.find(".syntax")
                ...
                .on("click", () => {
                    const shown = view
                        ...
                        .toggleClass("in")
                        .hasClass("in");
                    view.find(".syntax")[shown ? "addClass" : "removeClass"](
                        "in",
                    );
            ...
            view.find(".find")
                ...
                .on("click", () => {
                    ...
                    if (searchField?.[0]) searchField[0].focus();
                });
            view.find(".full")
                .off("click")
                .on("click", function () {
...
function onNewAudio(view) {
    ...
    (function updateBuffer() {
        ...
        view[0].querySelector(".seekbar-loaded").style.width =
            `${progress || 0}%`;
...
function modeFromShebang(text) {
    ...
    // remove flags (tokens starting with - or --), keep rest
    ...
    // shell scripts
    if (/^#!.*\b(ba|c|da|k|fi|tc|z)?sh$/.test(cleaned)) {
        return "shell";
    }
    ...
    for (const [exe, mode] of Object.entries(exes)) {
        // match shebang ending with the exe name (after flag stripping)
        if (new RegExp(`^#!.*\\b${exe}$`).test(cleaned)) {
            return mode;
        }
    }
...
function setEditorFontSize(size) {
    for (const sheet of arr(document.styleSheets)) {
        if (sheet.ownerNode?.id !== "css") continue;

        for (const rule of arr(sheet.cssRules)) {
...
function sortArrayByProp(arr, prop) {
    return arr.sort((a, b) => {
        ...
        if (result === 0)
            result = naturalSortWithNumbers(a.sortname, b.sortname);
        return result;
    });
}
...
function imgExtFromMime(mime) {
    let ret;
    for (const ext of Object.keys(droppy.imageTypes)) {
