...
module.exports = async function droppy(opts, isStandalone, dev, callback) {
  if (isStandalone) {
    log.logo(
      ...
      [blue("config"), "at", green(paths.config)].join(" "),
      [blue("files"), "at", green(paths.files)].join(" "),
    )
  }
  ...
  config.listeners.forEach((listener, i) => {
    ...
    sockets.forEach((socket) => {
      ...
      try {
        ...
        if (err.code !== "ENOENT") {
          return callback(new Error(`Unable to write to unix socket '${socket}': ${err.code}`))
        }
      }
    })
        ...
        createListener(onRequest, target.opts, (err, server) => {
          ...
          server.on("listening", () => {
            ...
            if (target.socket) {
              ...
              log.info("Listening on ", blue(`${proto}+unix://`) + cyan(server.address()))
            } else {
              // host + port
...
function sendFiles(sid, vId) {
  if (!clients[sid] || !clients[sid].views[vId] || !clients[sid].ws || !clients[sid].ws._socket)
    return
